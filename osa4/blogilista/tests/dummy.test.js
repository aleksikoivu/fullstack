const testConfig = require('../utils/list_helper').dummy
const totalLikes = require('../utils/list_helper').totalLikes
const favouriteBlog = require('../utils/list_helper').favouriteBlog
const mostBlogs = require('../utils/list_helper').mostBlogs
const mostLikes = require('../utils/list_helper').mostLikes

const listWithManyBlogs =  [
    { _id: "5a422a851b54a676234d17f7", title: "React patterns", author: "Michael Chan", url: "https://reactpatterns.com/", likes: 7, __v: 0 },
    { _id: "5a422aa71b54a676234d17f8", title: "Go To Statement Considered Harmful", author: "Edsger W. Dijkstra", url: "http://www.u.arizona.edu/~rubinson/copyright_violations/Go_To_Considered_Harmful.html", likes: 5, __v: 0 },
    { _id: "5a422b3a1b54a676234d17f9", title: "Canonical string reduction", author: "Edsger W. Dijkstra", url: "http://www.cs.utexas.edu/~EWD/transcriptions/EWD08xx/EWD808.html", likes: 12, __v: 0 },
    { _id: "5a422b891b54a676234d17fa", title: "First class tests", author: "Robert C. Martin", url: "http://blog.cleancoder.com/uncle-bob/2017/05/05/TestDefinitions.htmll", likes: 10, __v: 0 },
    { _id: "5a422ba71b54a676234d17fb", title: "TDD harms architecture", author: "Robert C. Martin", url: "http://blog.cleancoder.com/uncle-bob/2017/03/03/TDD-Harms-Architecture.html", likes: 0, __v: 0 },
    { _id: "5a422bc61b54a676234d17fc", title: "Type wars", author: "Robert C. Martin", url: "http://blog.cleancoder.com/uncle-bob/2016/05/01/TypeWars.html", likes: 2, __v: 0 }
]

describe('dummy test', () => {
    const blogs = []
    test('dummy returns one', () => {
        const result = testConfig(blogs)
        expect(result).toBe(1)
    })
})

describe('total likes', () => {
    const emptyList = []
    test('emptylist returns 0', () => {
        const result = totalLikes(emptyList)
        expect(result).toBe(0)
    })

    const listWithOneBlog = [
        {
        _id: '89dfs7yh932r98fa',
        title: 'Hello from one blog',
        author: 'Allu teki itse',
        url: 'No-can-do.co.uk',
        likes: 5,
        __v: 0
        }
    ]
    test('when list has only one blog equals the likes of that', () => {
        const result = totalLikes(listWithOneBlog)
        expect(result).toBe(5)
    })


    test('multiple blogs returns sum', () => {
        const result = totalLikes(listWithManyBlogs)
        expect(result).toBe(36)
    })
})

describe('favouriteblog', () => {
    test('most likes on a blog', () => {
        const result = favouriteBlog(listWithManyBlogs)
        expect(result).toEqual(listWithManyBlogs[2])
    })
})

describe('most blogs', () => {
    test('author with most blogs', () => {
        const result = mostBlogs(listWithManyBlogs)
        const correct = {
            author: listWithManyBlogs[5].author,
            blogs: 3
        }
        expect(result).toEqual(correct)
    })
})

describe('most likes on blogs total', () => {
    test('most likes on total blogs', () => {
        const result = mostLikes(listWithManyBlogs)
        const correct = {
            author: listWithManyBlogs[1].author,
            likes: 17
        }
        expect(result).toEqual(correct)
    })
})